...
var FolderScanner = {
    ...
    bindEvents: function() {
        ...
        // 关闭模态框
        var closeScanModal = document.getElementById('close-scan-modal');
        if (closeScanModal) {
        ...
        // 全选/取消全选
        var selectAllBtn = document.getElementById('select-all-orphaned');
        if (selectAllBtn) {
        ...
        var deselectAllBtn = document.getElementById('deselect-all-orphaned');
        if (deselectAllBtn) {
        ...
        // 导入选中文件
        var importBtn = document.getElementById('import-selected-files');
        if (importBtn) {
        ...
        // 模态框关闭按钮
        var modalCloses = document.querySelectorAll('#scan-result-modal .modal-close');
    ...
    startScan: function() {
        ...
        // 显示模态框
        var modal = document.getElementById('scan-result-modal');
        if (modal) {
            modal.style.display = 'flex';
        }

        // 显示加载状态
        document.getElementById('scan-result-content').style.display = 'block';
        document.getElementById('scan-result-data').style.display = 'none';
        ...
        jQuery.ajax({
            ...
            data: {
                action: 'scan_folder',
                base_dir: '/usr/uploads'
            },
            success: function(response) {
                if (response.success) {
                    self.displayScanResult(response.data);
                } else {
                    self.displayError(response.message || '扫描失败');
                }
            },
            error: function() {
                self.displayError('网络请求失败');
            }
        });
    },
    ...
    displayScanResult: function(data) {
        ...
        // 隐藏加载状态
        document.getElementById('scan-result-content').style.display = 'none';
        document.getElementById('scan-result-data').style.display = 'block';

        // 填充统计信息
        document.getElementById('scan-path').textContent = data.scanned_path;
        document.getElementById('total-files-system').textContent = data.total_files_in_system;
        document.getElementById('total-files-db').textContent = data.total_files_in_db;
        document.getElementById('orphaned-count').textContent = data.orphaned_count;

        // 如果有未录入的文件，显示文件列表
        if (data.orphaned_count > 0) {
            document.getElementById('orphaned-files-section').style.display = 'block';
            this.renderOrphanedFiles(data.orphaned_files);
        } else {
            document.getElementById('orphaned-files-section').style.display = 'none';
        }
    },

    renderOrphanedFiles: function(files) {
        var self = this;
        var listContainer = document.getElementById('orphaned-files-list');
        listContainer.innerHTML = '';

        files.forEach(function(file, index) {
            var fileItem = document.createElement('div');
            fileItem.className = 'scan-file-item';

            var checkbox = document.createElement('input');
            checkbox.type = 'checkbox';
            checkbox.className = 'scan-file-checkbox';
            ...
            var icon = document.createElement('div');
            icon.className = 'scan-file-icon';
            icon.textContent = self.getFileIcon(file);

            var infoDiv = document.createElement('div');
            infoDiv.className = 'scan-file-info';

            var nameSpan = document.createElement('span');
            nameSpan.className = 'scan-file-name';
            nameSpan.textContent = file.name;

            var metaSpan = document.createElement('span');
            metaSpan.className = 'scan-file-meta';
            metaSpan.textContent = file.size_formatted + ' | ' + file.modified_formatted;

            var pathSpan = document.createElement('div');
            pathSpan.className = 'scan-file-path';
            pathSpan.textContent = file.relative_path;
    ...
    selectAllFiles: function() {
        var checkboxes = document.querySelectorAll('.scan-file-checkbox');
        checkboxes.forEach(function(checkbox) {
    ...
    updateSelectedFiles: function() {
        ...
        var checkboxes = document.querySelectorAll('.scan-file-checkbox:checked');
        checkboxes.forEach(function(checkbox) {
            var index = parseInt(checkbox.value);
            if (self.scanData && self.scanData.orphaned_files[index]) {
                self.selectedFiles.push(self.scanData.orphaned_files[index]);
            }
        });
    },
    ...
    importSelectedFiles: function() {
        ...
        if (!confirm('确定要导入 ' + self.selectedFiles.length + ' 个文件到数据库吗？')) {
            return;
        }
        ...
        jQuery.ajax({
            ...
            data: {
                action: 'import_files',
                files: JSON.stringify(self.selectedFiles)
            },
            success: function(response) {
                if (response.success) {
                    var data = response.data;
                    var message = '导入完成！\n';
                    message += '成功导入: ' + data.imported + ' 个文件\n';
                    if (data.failed > 0) {
                        message += '失败: ' + data.failed + ' 个文件\n';
                        if (data.errors.length > 0) {
                            message += '\n错误信息:\n' + data.errors.slice(0, 5).join('\n');
                            if (data.errors.length > 5) {
                                message += '\n... 及其他 ' + (data.errors.length - 5) + ' 个错误';
                            }
                        }
                    }
                    alert(message);
...
window.FolderScanner = FolderScanner;

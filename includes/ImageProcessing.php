...
class MediaLibrary_ImageProcessing
{
    ...
    public static function compressImageByPath($filePath, $quality = 80, $compressMethod = 'gd', $replaceOriginal = true, $outputFormat = 'original', $customName = '')
    {
        ...
        if (strpos($mime, 'image/') !== 0) {
            return ['success' => false, 'message' => '只能压缩图片文件'];
        }
        ...
        if ($replaceOriginal) {
            if ($outputFormat === 'original') {
                ...
                // 先压缩到临时文件
                $result = self::compressImageWithMethod($filePath, $tempPath, $quality, $compressMethod, $outputFormat, $mime);
                ...
                if (!@unlink($filePath) || !rename($tempPath, $filePath)) {
                    @unlink($tempPath);
                    return ['success' => false, 'message' => '文件替换失败'];
                }
                ...
                // 删除原文件并重命名临时文件
                $newPath = $pathInfo['dirname'] . '/' . $pathInfo['filename'] . '.' . $newExt;
                @unlink($filePath);
